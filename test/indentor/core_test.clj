...
(deftest parse-and-act-test
  ...
  (in-test-env (is (= {:ext :js :style :space :size 1}
                      (do (parse-and-act (args "set -e clj -s space"))
                          (parse-and-act (args "set -e js -s space"))
                          (parse-and-act (args "get -e clj"))))
                   "TODO: Fix different ext overwriting each other")))
